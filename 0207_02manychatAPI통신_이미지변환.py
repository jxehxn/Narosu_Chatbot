...
async def process_ai_response(sender_id: str, user_message: str):
    try:
        ...
        # ✅ bot_response가 dict가 아닌 경우 JSON 변환 시도
        if isinstance(bot_response, str):
            try:
                bot_response = json.loads(bot_response)
            except json.JSONDecodeError as e:
                print(f"❌ [ERROR] bot_response JSON 변환 실패: {e}")
                return

        # ✅ 최종적으로 bot_response가 dict가 아니면 오류 처리
        if not isinstance(bot_response, dict):
            print(f"❌ [ERROR] bot_response는 dict 형태여야 합니다. 현재 타입: {type(bot_response)}")
            return

        # ✅ ManyChat API로 최종 메시지 전송
        if "response" in bot_response:
            send_message(sender_id, bot_response)
            print(f"🤖 [AI 응답 전송 완료]: {bot_response['response']}")
        else:
            print(f"❌ AI 응답 오류 발생")
...
def send_message(recipient_id: str, message_data: dict):
    ...
    # ✅ 1. `message_data`가 None 또는 비어 있는지 확인
    if not message_data:
        print("❌ [ERROR] `message_data`가 None 또는 비어 있습니다.")
        return

    # ✅ 2. `message_data`가 문자열이면 JSON 변환
    if isinstance(message_data, str):
        try:
            message_data = json.loads(message_data)  # 문자열을 JSON(dict)로 변환
        except json.JSONDecodeError as e:
            print(f"❌ [ERROR] `message_data` JSON 변환 실패: {e}")
            return

    # ✅ 3. `message_data`가 JSON(dict)인지 최종 확인
    if not isinstance(message_data, dict):
        print(f"❌ [ERROR] `message_data`는 JSON(dict) 형태여야 합니다. 현재 타입: {type(message_data)}")
        return

    # ✅ 4. ManyChat API 형식에 맞는 payload 생성
    payload = {
    ...
    # ✅ 5. LLM 응답 추가 (텍스트 메시지만 추가)
    if "response" in message_data and isinstance(message_data["response"], str) and message_data["response"].strip():
        payload["data"]["content"]["messages"].append({
            "type": "text",
            "text": message_data["response"]
        })

    # ✅ 6. 검색된 상품 리스트 추가
    if "results" in message_data and isinstance(message_data["results"], list):
        for item in message_data["results"]:
            if isinstance(item, dict) and "제목" in item and "가격" in item and "배송비" in item:
                text_message = f"{item['제목']}\n- 가격: {item['가격']}원\n- 배송비: {item['배송비']}원"
                payload["data"]["content"]["messages"].append({"type": "text", "text": text_message})

                # ✅ 이미지 추가 (상품에 이미지 URL이 있을 경우)
                if item.get("이미지"):
                    payload["data"]["content"]["messages"].append({
                        "type": "image",
                        "url": item["이미지"]
    ...
    # ✅ 7. 요청된 JSON을 ManyChat이 요구하는 형식과 비교 (디버깅용)
    print(f"🔍 [DEBUG] ManyChat API로 전송할 payload: {json.dumps(payload, indent=2, ensure_ascii=False)}")
